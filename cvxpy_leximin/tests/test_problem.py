...
class TestProblem(BaseTest):
    ...
    def test_solve_1(self) -> None:
        ...
        self.assertAlmostEqual(a[0].value, 1, places=3)
        self.assertAlmostEqual(a[1].value, 1, places=3)
        self.assertAlmostEqual(a[2].value, 0, places=3)
        self.assertAlmostEqual(a[3].value, 0, places=3)
    ...
    def test_solve_2(self) -> None:
        ...
        problem = Problem(Leximin([utility_Alice, utility_George]), constraints=feasible_allocation)
        problem.solve()
    ...
    def test_solve_3(self) -> None:
        ...
        utility_George = 1 * (1 - a[0]) + 1 * (1 - a[1]) + 1 * (1 - a[2]) + 0 * (1 - a[3])
        problem = Problem(Leximin([utility_Alice, utility_George]), constraints=feasible_allocation)
        problem.solve()
